...
//SETTING UP DATABASE
var mongodburl =
  "mongodb+srv://covid_cluster_user:randomlygenerated@covidpatients.cf3et.mongodb.net/covidPatients?retryWrites=true&w=majority";
    ...
    if (err) res.send(err);
      ...
      PatientTreament.findOne({ date: req.body.date }, function (err, found) {
        if (err) {
            ...
            function (err, newtreatment) {
              if (err) res.send(err);
                ...
                Medicinegiven.create({ medicinegiven }, function (err, record) {
                  if (err) res.send(err);
                  else newtreatment.medicine.push(record);
                  newtreatment.save();
                });
          ...
          Medicinegiven.create({ medicinegiven }, function (err, record) {
            if (err) res.send(err);
            else found.medicine.push(record);
            found.save();
            res.redirect("/api/patient/" + req.params.id);
          });
        }
...
app.get("/patient/:id/discharge", function (req, res) {
  User.findById(req.params.id, function (err, patient) {
    if (err) res.send("No such patient");
    else
...
app.post("/patient/:id/discharge", function (req, res) {
  User.findByIdAndUpdate(
    req.params.id,
    { "forPatient.treatment.dischargedate": req.body.date },
    function (err, updatedPatient) {
      if (err) res.redirect("/api/patient/" + req.params.id);
      else {
          ...
          if (err) {
            ...
            if (req.body.bed == "covid") {
              Bed.findOneAndUpdate(
                {},
                {
                  "occupied.covid": items[0].occupied.covid - 1,
                  "available.covid": items[0].available.covid + 1,
                },
                function (err, result) {
                  if (err) {
                    console.log(err);
              ...
              Bed.findOneAndUpdate(
                {},
                {
                  "occupied.noncovid": items[0].occupied.noncovid - 1,
                  "available.noncovid": items[0].available.noncovid + 1,
                },
                function (err, result) {
                  if (err) {
                    console.log(err);
              ...
              Bed.findOneAndUpdate(
                {},
                {
                  "occupied.icu": items[0].occupied.icu - 1,
                  "available.covid": items[0].available.icu + 1,
                },
                function (err, result) {
                  if (err) {
                    console.log(err);
...
app.get("/patient/:id/admit", function (req, res) {
  User.findById(req.params.id, function (err, patient) {
    if (err) res.send("No such patient");
    else res.render("admit_or_discharge", { patient: patient, type: "admit" });
  });
});
...
app.post("/patient/:id/admit", function (req, res) {
  User.findByIdAndUpdate(
    req.params.id,
    {
      "forPatient.treatment.admitdate": req.body.date,
      "forPatient.treatment.treatedby": req.body.doctor,
      "forPatient.treatment.covidpositive": req.body.corona,
    },
    function (err, updatedPatient) {
      if (err) res.redirect("/api/patient/" + req.params.id);
      else {
...
app.get("/patient/:id/update", function (req, res) {
  User.findById(req.params.id, function (err, patient) {
    if (err) res.send("No such patient");
    else res.render("edit_patient_details", { patient: patient });
  });
});
...
app.post("/patient/:id/update", function (req, res) {
  ...
  User.findByIdAndUpdate(req.params.id, newuser, function (
    ...
    if (err) res.redirect("/api/patient/" + req.params.id);
    else res.redirect("/api/patient/" + req.params.id);
  });
});
...
app.get("/patient/:id", function (req, res) {
  User.findById(req.params.id, function (err, patient) {
    if (err) res.send("No such patient");
    else res.render("home", { patient: patient });
  });
});
...
app.get("/staff/:id/addintime", function (req, res) {
  User.findById(req.params.id, function (err, staff) {
    if (err) res.send(err);
    else res.render("addtime", { time: "intime", id: req.params.id });
  });
});
...
app.post("/staff/:id/addintime", function (req, res) {
  User.findById(req.params.id, function (err, staff) {
    if (err) res.send(err);
      ...
      Workhour.create(
        { date: req.body.date, checkIn: req.body.intime, checkOut: null },
        function (err, work) {
          if (err) {
            console.log(err);
          } else {
...
app.post("/staff/:id/relieve", function (req, res) {
  User.findByIdAndUpdate(
    req.params.id,
    { "forStaff.relievedOn": req.body.relieve },
    function (err, staff) {
      if (err) res.send(err);
      else {
        res.redirect("/api/staff/" + req.params.id);
...
app.get("/staff/:id", function (req, res) {
  User.findById(req.params.id, function (err, staff) {
    if (err) res.send(err);
    else res.render("home_staff", { staff: staff, id: req.params.id });
  });
});

app.get("/api/staff/:id", function (req, res) {
  User.findById(req.params.id, function (err, staff) {
    if (err) res.send("No such staff");
    else res.send(staff);
  });
});
    ...
    if (err) {
      ...
      if (req.body.item == "gloves") {
        Inventory.findOneAndUpdate(
          {},
          { gloves: items[0].gloves + Number(req.body.value) },
          function (err, result) {
            if (err) {
              console.log(err);
            } else {
        ...
        Inventory.findOneAndUpdate(
          {},
          { testingkit: items[0].testingkit + Number(req.body.value) },
          function (err, result) {
            if (err) {
              console.log(err);
            } else {
        ...
        Inventory.findOneAndUpdate(
          {},
          { syringes: items[0].syringes + Number(req.body.value) },
          function (err, result) {
            if (err) {
              console.log(err);
            } else {
        ...
        Inventory.findOneAndUpdate(
          {},
          { salinebottles: items[0].salinebottles + Number(req.body.value) },
          function (err, result) {
            if (err) {
              console.log(err);
            } else {
        ...
        Inventory.findOneAndUpdate(
          {},
          { masks: items[0].masks + Number(req.body.value) },
          function (err, result) {
            if (err) {
              console.log(err);
            } else {
        ...
        Inventory.findOneAndUpdate(
          {},
          { ppekits: items[0].ppekits + Number(req.body.value) },
          function (err, result) {
            if (err) {
              console.log(err);
            } else {
